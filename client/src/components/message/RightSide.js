...
const RightSide = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // ✅ Emitir evento stopTyping después de enviar
  ...
  // ✅ Emit typing cuando el usuario escribe
  useEffect(() => {
    if (text) {
    ...
    <>
      <div className="message_header" style={{ cursor: 'pointer' }}>
        {user.length !== 0 && (
          <UserCard user={user}>
            <div>
              <i className="fas fa-arrow-left" onClick={handleAudiopaginainterior} />
              <i className="mx-3" />
      ...
      >
        <div className="chat_display" ref={refDisplay}>
          ...
          {isTyping && (
            <div className="chat_row other_message">
              <div className="typing_indicator">Escribiendo...</div>
            </div>
          )}
        </div>
      </div>
